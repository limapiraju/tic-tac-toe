...
def recebe_jogada(vez_do_jogador):  
    ...
    if vez_do_jogador:
        ...
        while True:
            ...
            imprime("Valor inválido!", antes = False)
...
def imprime(text, antes = True, depois = True, token = "-", size = 50):
    """Implementa a função print com umas frescuras antes e depois do texto principal.

    antes (True): imprime hífens (ou outros tokens) antes do texto;
    depois (True): imprime hífens (ou outros tokens) depois do texto.
    """
